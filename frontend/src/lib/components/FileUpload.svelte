<script lang="ts"></script>
    ...
    async function uploadFilesWithPrompt() {
        ...
        try {
            // Use the new chat API for file uploads
            const response = await newChatApi.uploadFilesToChat(
                files, 
                prompt.trim(), 
                chatId
            );
            ...
            if (response.success) {
                // For new chat uploads, we might get a chat_id back
                const resultChatId = chatId || response.chat_id;
